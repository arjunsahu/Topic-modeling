...
//occupation like '%".$centre."%' and rank1  like '%".$rank1."%'   and type_of_buyer like '%".$vechile_name."%' and brand_intended like '%".$intender."%' and  vichel like '%".$vechile_code."%' and vichel_name  like '%".$concept."%' and pj  like '%".$mention."%'
...
$concept= $_POST["concept"]?: '';//vichel_name
...
<form method="post" action="">
  <div class="top  b-b" style="background-color:#F0F0F0">
   <div class="box" style=" margin-bottom: 0rem;">
		...
		<div class="row">
		    <div class="col-md-8">
          <h2>Section B1 - FeedBack On the  Vehicle Exposed Combine Pune & Bangalore</h2>
          <small>Interviewer to code the vehicle to be Exposed Combine Pune & Bangalore.</small>
		   </div>
		     <div class="col-md-4">
...
while($r = mysql_fetch_array($word)) {
			...
			<!-- Section B -->
			<div class="col-md-12 text-center" style="padding-bottom: 15px;"> 
					<span  class="btn btn-sm btn-outline rounded b-accent">Section B1 </span> 
    ...
    <option value="aspect_like">Aspects Liked</option>
    <option value="details_like">Details Liked</option>
    <option value="benefit_like">Benfits Liked</option>
	<option value="aspect_dislike">Aspects Disliked</option>
    <option value="details_dislike">Details Disliked</option>
	   
    <option value="problem_dislike">Aspects Disliked Problem</option>
	  </optgroup>
	...
	<div class="form-inline">
			...
			<div class="col-sm-4">
			<small class="label label-default label-as-badge">Select mention </small>	
			 
			 <div class="form-group row">
            
            <div class="col-sm-10">
                <select class="form-control c-select"  name="mention_no">
				
	
                  <option selected disabled>Select mention</option>
				  <option value="">All</option>
			...
			<!-- Section B -->
			<div class="col-md-12 text-center" style="padding-bottom: 15px;"> 
					<span  class="btn btn-sm btn-outline rounded b-accent">Section B1 rank1 for tvs Entorq </span> 
		...
		<div class="form-inline">
			...
			<small class="label label-default label-as-badge">Select Analysis</small>	
             ...
             <div class="col-sm-4">
			<small class="label label-default label-as-badge">Select Rank1 for all togeather BO2,B04,BO6  </small>	
			 
			 <div class="form-group row">
            
            <div class="col-sm-10">
                <select class="form-control c-select"  name="ent">
				
	
                  <option selected disabled>Select Rank1 </option>
				  <option value="11">Rank1 for BO2,B04,BO6</option>
...
$q_add="and vichel in (1) and r_name like '%ENT%' and reason_choosing_ranked_1_vname like '%ENT%' and ridd_name like '%ENT%' ";
$concept='';
}
...
//Nishank
$result=mysql_query("select COUNT(DISTINCT serialno),serialno from $table_name  where occupation like '%".$centre."%' and marital_status like '%".$age_group."%' and mention like '%".$mentio_no."%' and panel_no like '%".$panel."%' and age like '%".$age."%' and type_of_buyer like '%".$vechile_name."%' and brand_intended like '%".$intender."%' and  vichel like '%".$vechile_code."%' and vichel_name  like '%".$concept."%' and pj  like '%".$mention."%'  $q_add" ) or
die(mysql_error());
$data=mysql_fetch_array($result);

$deno= $data[0];
//$deno= 48;
  echo "<script>console.log( 'First : " . $deno . "' );</script>";
//if ($deno!=159 AND  $panel == 1 ){$sub=155-$deno;$deno=$deno+$sub;} else{$deno= $data[0];}
...
foreach ($analysis as $value) { $wheel[] = "$value REGEXP 'wheel|brake|alloy|tyre|BREAKS'" ;}
...
foreach ($analysis as $value) { $mileage[] = "$value REGEXP 'mil|average|kmps'" ;}
foreach ($analysis as $value) { $power[] = "$value REGEXP ' power |engine|start|kick|pick'" ;}
...
foreach ($analysis as $value) {  $cost[] = "$value REGEXP 'cost|price|money|rupee|expens|budg'  " ;}
foreach ($analysis as $value) {  $body[] = "$value REGEXP 'heav|wei'  " ;}
foreach ($analysis as $value) {  $brand[] = "$value REGEXP 'brand|compa'  " ;}

foreach ($analysis as $value) {  $tech[] = "$value REGEXP 'tech'  " ;}
foreach ($analysis as $value) {  $Suitable_for[] = "$value REGEXP ' men| man|young|Child|Lady|women|boy'  " ;}
foreach ($analysis as $value) {  $battery[] = "$value REGEXP 'batt|usb|charge'  " ;}
foreach ($analysis as $value) {  $bluetooth[] = "$value REGEXP 'bluet|blue t'  " ;}
//Nishank whole query Change
$v1_q = mysql_query("
select  count(DISTINCT serialno) from expo_savvy where (".implode($head_light," OR ")." ) and  occupation like '%".$centre."%' and rank1  like '%".$rank1."%'  and ( rank2  like '%".$rank2."%' and  rank3  like '%".$rank3."%' ) and marital_status like '%".$age_group."%' and mention like '%".$mentio_no."%' and panel_no like '%".$panel."%' and age like '%".$age."%' and type_of_buyer like '%".$vechile_name."%' and brand_intended like '%".$intender."%' and  vichel like '%".$vechile_code."%' and vichel_name  like '%".$concept."%' and pj  like '%".$mention."%'  $q_add
 union all 
...
$(function () {
    $('#sentiment').highcharts({
        ...
        subtitle: {
             text: 'Base :- <?php echo $deno;?> | Panel :- <?php echo $panel?:"ALL";?> | Vechile_name :- <?php echo $concept?:"ALL";?> '
        },
	xAxis: {
        //nishank
...
<div class="row">
       ...
       implode('&analysis[]=', array_map('urlencode', $analysis));?>
...
console.log("dashboard_tree2_v.php?centre=<?php echo $centre;?>&q_add=<?php echo $q_add;?>&vichel_group=<?php echo $vichel_group;?>&age_group=<?php echo $age_group;?>&panel=<?php echo $panel;?>&age=<?php echo $age;?>&vechile_name=<?php echo $vechile_name;?>&intender=<?php echo $intender;?>&concept=<?php echo $concept;?>&table_name=<?php echo $table_name; ?>&mention=<?php echo $mention ;?>&vechile_code=<?php echo $vechile_code;?>&mention_no=<?php echo $mention_no;?>&vechicle_before=<?php echo $vechicle_before;?>&vechicle_after=<?php echo $vechicle_after; ?>&rank1=<?php echo $rank1; ?>&rank3=<?php echo $rank3 ;?>&rank2=<?php echo $rank2;?>&rank1_a=<?php echo $rank1_a;?><?php  
      echo '&analysis[]=' . 
       implode('&analysis[]=', array_map('urlencode', $analysis));?> ");
// load the external data

d3.csv("dashboard_tree2_v.php?centre=<?php echo $centre;?>&q_add=<?php echo $q_add;?>&vichel_group=<?php echo $vichel_group;?>&age_group=<?php echo $age_group;?>&panel=<?php echo $panel;?>&age=<?php echo $age;?>&vechile_name=<?php echo $vechile_name;?>&intender=<?php echo $intender;?>&concept=<?php echo $concept;?>&table_name=<?php echo $table_name; ?>&mention=<?php echo $mention ;?>&vechile_code=<?php echo $vechile_code;?>&mention_no=<?php echo $mention_no;?>&vechicle_before=<?php echo $vechicle_before;?>&vechicle_after=<?php echo $vechicle_after; ?>&rank1=<?php echo $rank1; ?>&rank3=<?php echo $rank3 ;?>&rank2=<?php echo $rank2;?>&rank1_a=<?php echo $rank1_a;?><?php  
      echo '&analysis[]=' . 
       implode('&analysis[]=', array_map('urlencode', $analysis));?> ", function(error, data) {
